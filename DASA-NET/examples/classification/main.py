...
if __name__ == "__main__":
    ...
    # torch.cuda.set_per_process_memory_fraction(0.4, 0)  # 限制显存使用为 X0%
    # torch.cuda.set_per_process_memory_fraction(0.4, 1)  # 限制显存使用为 X0%
    # torch.cuda.set_per_process_memory_fraction(0.4, 2)  # 限制显存使用为 X0%

    # os.environ['CUDA_VISIBLE_DEVICES'] = "1" # 用这种方法指定GPU
    ...
    # cfg.sync_bn = cfg.world_size > 1  # 注释下面两行，恢复本行则启用分布式训练
    cfg.mp = False  # 禁用多进程训练

    cfg.sync_bn = False  # 关闭同步批量归一化，适用于单GPU训练

    #ADD  Ensure consistency between distributed and multi-processing settings
    if not cfg.mp:
